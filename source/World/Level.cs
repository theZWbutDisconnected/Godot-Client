...
{
	private readonly Dictionary<ChunkCoordIntPair, List<MeshInstance3D>> _chunkMeshes = new();
	...
	private void BuildChunkMeshAsync(ChunkData chunk)
	{
		...
		var groups = new Dictionary<MeshGroupKey, Tessellator>();
		...
		for (var y = 0; y < ChunkData.Height; y++)
		{
			...
			for (var z = 0; z < ChunkData.Depth; z++)
			{
				...
				bool isLiquid = block is LiquidTile;
				int texId = block.TexId;
				bool animated = TextureAtlas.IsAnimated(texId);
		...
		var meshes = new Godot.Collections.Array<MeshInstance3D>();
	...
	private void ApplyChunkMesh(int chunkX, int chunkZ, Godot.Collections.Array<MeshInstance3D> meshes)
	{
...
#nullable disable
	...
	private struct MeshGroupKey : IEquatable<MeshGroupKey>
	{
		...
		public override bool Equals(object obj) => obj is MeshGroupKey other && Equals(other);
