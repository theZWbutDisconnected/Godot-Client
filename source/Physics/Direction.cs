...
{
    public static readonly Direction DOWN = new Direction(0, 1, -1, "down", AxisDirection.NEGATIVE, Axis.Y, new Vec3i(0, -1, 0));
    public static readonly Direction UP   = new Direction(1, 0, -1, "up",   AxisDirection.POSITIVE, Axis.Y, new Vec3i(0,  1, 0));
    public static readonly Direction NORTH= new Direction(2, 3,  2, "north",AxisDirection.NEGATIVE, Axis.Z, new Vec3i(0,  0,-1));
    public static readonly Direction SOUTH= new Direction(3, 2,  0, "south",AxisDirection.POSITIVE, Axis.Z, new Vec3i(0,  0, 1));
    public static readonly Direction WEST = new Direction(4, 5,  1, "west", AxisDirection.NEGATIVE, Axis.X, new Vec3i(-1, 0, 0));
    public static readonly Direction EAST = new Direction(5, 4,  3, "east", AxisDirection.POSITIVE, Axis.X, new Vec3i( 1, 0, 0));

    private static readonly Direction[] _values = new Direction[6];
    private static readonly Direction[] _horizontals = new Direction[4];
    private static readonly Dictionary<string, Direction> _nameLookup = new Dictionary<string, Direction>();
    ...
    static Direction()
    {
        foreach (var f in new[] { DOWN, UP, NORTH, SOUTH, WEST, EAST })
        {
            ...
            if (f.Axis.IsHorizontal)
                _horizontals[f._horizontalIndex] = f;
            _nameLookup[f._name] = f;
        }
    }

    private readonly int _index;
    private readonly int _oppositeIndex;
    private readonly int _horizontalIndex;
    ...
    public static Direction ByName(string name) =>
        name == null ? null : _nameLookup.TryGetValue(name.ToLowerInvariant(), out var f) ? f : null;

    public static Direction GetFront(int index) => _values[Mathf.Abs(index % _values.Length)];

    public static Direction GetHorizontal(int index) => _horizontals[Mathf.Abs(index % _horizontals.Length)];

    public static Direction FromAngle(double angle) =>
        GetHorizontal(Mathf.FloorToInt(angle / 90.0 + 0.5) & 3);

    public static Direction Random(Random rand) => _values[rand.Next(_values.Length)];
    ...
    public static Direction GetFacingFromVector(float x, float y, float z)
    {
        ...
        {
            float dot = x * f.DirectionVec.X + y * f.DirectionVec.Y + z * f.DirectionVec.Z;
            if (dot > bestDot)
            {
                bestDot = dot;
                best = f;
